...
static void BM_cu_bit_operation_o3(benchmark::State &state)
{
    int N = state.range(0);
...
BENCHMARK(BM_cu_bit_operation_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);


static void BM_bit_operation_o3(benchmark::State &state)
{
    int N = state.range(0);
    state.SetComplexityN(state.range(0));
    vector<int> shape = {N, N};
...
BENCHMARK(BM_bit_operation_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);

static void BM_omp_bit_operation_o3(benchmark::State &state)
{
    int N = state.range(0);
    state.SetComplexityN(state.range(0));
    vector<int> shape = {N, N};
...
BENCHMARK(BM_omp_bit_operation_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);
