...
static void BM_comparison_o3(benchmark::State &state)
{
    int N = state.range(0);
...
static void BM_cu_comparison_o3(benchmark::State &state)
{
    int N = state.range(0);
    state.SetComplexityN(state.range(0));
    vector<int> shape = {N, N};
...
static void BM_omp_comparison_o3(benchmark::State &state)
{
    int N = state.range(0);
    state.SetComplexityN(state.range(0));
    vector<int> shape = {N, N};
...
BENCHMARK(BM_cu_comparison_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);
BENCHMARK(BM_comparison_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);
BENCHMARK(BM_omp_comparison_o3)->DenseRange(16, 2048, 16)->Complexity(benchmark::oAuto);
